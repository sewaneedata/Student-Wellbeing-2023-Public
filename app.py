...
HMS = pd.read_csv(Path(__file__).parent / 'HMS_Private.csv', low_memory=False)
...
#Key insights, text insights and 5 questions (q's might be moved elsewhere)
...
#Substance Use 4 questions, binge question and alc problem both don't work yet

        ui.nav("Substance Use",
            ui.h2("Substance Use"),
           
            ui.input_select("x1", "Select A Demographic", SubUseDemo),
            ui.h4("Have you consumed any alcohol in the past 2 weeks?"),
            ui.output_plot("alcplot1"),
           
            ui.input_select("x2", "Select A Demographic", SubUseDemo),
            ui.h4("Over the past 30 days, about how many cigarettes did you smoke per day?"),
            ui.output_plot("smokeplot1"),

            ui.markdown(script.BDdef),
            ui.input_select("x3", "Select A Demographic", SubUseDemo),
            ui.h4("How many times have you binge drank in the past 2 weeks?"),
            ui.output_plot("bingeplot1"),

            ui.input_select("x4", "Select A Demographic", SubUseDemo),
            ui.h4("Alcohol is a problem at my university(2022 only)"),
            ui.output_plot("alcproblem1"),
            ),
        
#Sexual Assault check what is asked for question 1 in the codebook

        ui.nav("Sexual Assault",
            ui.h2("Sexual Assault"),
            ui.markdown(script.SAdef),
            ui.h2("In the past 12 months(?), have you been sexually assaulted?"),
            ui.output_plot("SAplot1"),

            ui.h2("Who sexually assaulted you?(2019)"),
            ui.output_plot("SAplot3"),

            ui.h2("Who sexually assaulted you?(2020)"),
            ui.output_plot("SAplot4"),

            ui.h2("Who did you tell?(2019)"),
            ui.output_plot("SAplot2"),

            ui.h2("Who did you tell?(2020)"),
            ui.output_plot("SAplot5"),


            ),

#Suicidal Ideation 2 questions rework bottom one

        ui.nav("Suicidal Ideation",
            ui.h2("Suicidal Ideation"),
            ui.markdown(script.SIdef),
            ui.input_select("SI1", "Select a comparrison score", CompScore),
            ui.h4("In the past year, did you ever seriously think about attempting suicide?"),
            ui.output_plot("keyplot6"),

            ui.h4("In the past year, did you attempt suicide? (of students who said 'yes' to the first question)"),
            ui.output_plot("keyplot7"),
            ),
...
def server(input, output, session):
    ...
    #HMS['belong_8_'] creates a new column in the data that only has students who responded to the question('I feel isolated from campus life') in the survey
...
#Social Isolation Output
    ...
    #HMS['belong_1_'] creates a new column of data with only people who responded to the question ("I see myself as a part of the campus community")
    #'neg_belong1' reverses the data so the likert scale could start with disagree at first on the axis(as with all data that starts with 'neg')
    def campus1():
    ...
    def campus4():
        ...
        axd.set(xlabel = input.x8(), ylabel = 'Belong at school', title = 'I feel I belong at this school')
...
#Mental Health Outputs
    ...
    def mhplot1():
        ...
        #changes the average of the axis into percentages
    ...
    def mhplot2():
        ...
        for bar in ax.get_children():
            if isinstance(bar, mpl.patches.Rectangle) and bar.xy != (0,0):
                count = int(bar.get_height())
                x = bar.xy[0]
                #creates numbers on the graph for the count plot
...
#Substance Use Outputs

    @output
    @render.plot
    @reactive.event(input.year, input.x1)
    def alcplot1():
        sns.barplot(data= HMS, x= input.x1(), y='alc_any', errorbar = None)
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0))
        plt.xticks( rotation = 40)

    @output
    @render.plot
    @reactive.event(input.year, input.x2)
    
    def smokeplot1():
        aax=sns.barplot(data= HMS, x= input.x2(), y='smok_freq', errorbar = None)
        aax.set_yticks(range(5))
        aax.set_yticklabels(["0 cigarettes", 'Less than 1 cigarettes', '1-5 cigarettes', 'About one-half pack', '1 or more packs'])
        plt.title('Over the past 30 days, about how many cigarettes did you smoke per day?')
        plt.xlabel(input.x2())
        plt.xticks( rotation = 40)

    @output
    @render.plot
    @reactive.event(input.year, input.x3)
    
    def bingeplot1():
        ay= sns.barplot(data= HMS.where(HMS['binge_d']!=0), x= input.x3(),y='binge_d', errorbar=None)
        ay.set(ylabel = 'Binge Drink', xlabel = input.x3()),
        ay.set_yticks(range(7))
        ay.set_yticklabels(["",'0 times', '1 time', '2 times', '3-5 times', '6 to 9 times', '10 or more times'])
        plt.xticks( rotation = 40)

    
    @output
    @render.plot
    @reactive.event(input.year, input.x4)

    def alcproblem1():

        ax = sns.barplot(data = HMS.where(HMS['alc_prob']!=0), x = input.x4(), y = 'alc_prob2', errorbar=None)
        ax.set(xlabel = input.x4(), title = 'I think alcohol is a problem on my campus')
        ax.set_yticks(range(7))
        ax.set_yticklabels(['','Strongly Disagree', 'Disagree', 'Somewhat Disagree', 'Somewhat Agree', 'Agree', 'Strongly Agree'])
        plt.xticks( rotation = 40)

#Sexual Assault Output (two outputs, check Q1 codebook)
    ...
    def SAplot1():
        sa=sns.countplot(data= HMS.where(HMS['assault_sex']!=0), x='survey',hue='assault_sex',palette="BuPu")
        plt.legend(title='Sexual Assault', loc='upper left', labels=['Yes', 'No'])
        for bar in sa.get_children():
            if isinstance(bar, mpl.patches.Rectangle) and bar.xy != (0,0):
                count = int(bar.get_height())
                x = bar.xy[0]
                sa.annotate(count, (x + 0.099, count + 0.05))

    @output
    @render.plot

    def SAplot2():
        sns.countplot(data=HMS.where(HMS['survey']==2019), y='sa_disclose',palette="plasma")

    @output
    @render.plot 

    def SAplot3():
        sns.countplot(data=HMS.where(HMS['survey']==2019), y='sa_who',palette="plasma")
    ...
    def SAplot4():
        sns.countplot(data=HMS.where(HMS['survey']==2020), y='sa_who',palette="plasma")
    ...
    def SAplot5():
        sns.countplot(data=HMS.where(HMS['survey']==2020), y='sa_disclose',palette="plasma")

       
    
#Suicidal Ideation Outputs

    @output
    @render.plot
    @reactive.event(input.SI1)

    def keyplot6():

        af = sns.barplot(data = HMS, y = input.SI1(), x = 'sui_idea', errorbar = None)
        af.set_xticklabels(['No', 'Yes'])
        af.set(ylabel = input.SI1(), xlabel = 'Suicide Ideation')
    ...
    def keyplot7():
        ...
        #Percentage of students who attempted suicide with a suicide ideation

        HMS['sui_idea_check'] = HMS.apply(lambda row:
                                 row['sui_idea'] > 0, axis = 1)

        ax = sns.barplot(data = HMS[HMS['sui_idea_check']], y = 'GenderID', x = 'sui_att', errorbar = None)
        plt.gca().xaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0))
        ax.set(ylabel = 'Gender', xlabel = 'Suicide Ideation')
